...
def process_grid(grid_data):
    grids = {}
    smallest_point = [grid_data["features"][0]['properties']['id'],
                      grid_data["features"][0]['geometry']['coordinates'][0][1]]
...
def result_output(result_dict, language_dict):
        ...
        if len(list(top_10_dict)) >= 10:
            for j in range(10):
                if list(top_10_dict)[j] in language_dict:
                    top_10_list.append(
                        language_dict[list(top_10_dict)[j]] + '-' + str(top_10_dict[list(top_10_dict)[j]]))
                else:
    ...
    # Process DataFrame table
    pd.set_option('display.colheader_justify', 'center')
    pd.set_option('max_colwidth', 200)
    df = pd.DataFrame(result_list,
                      columns=["Cell", "#Total Tweets", "#Number of language used", "#Top 10 Languages & #Tweets"])
    # replace cell name
    replace_name_cell = df.replace({"Cell":{23: "C4", 22: "B4", 21: "A4", 20: "D3", 19: "C3", 18: "B3", 17: "A3", 16: "D2", 15: "C2", 14: "B2", 13: "A2", 12: "D1", 11: "C1", 10: "B1", 9: "A1", 24: "D4"}})
    # ascending sort by cells
    ascending_order_table = replace_name_cell.sort_values(by=["Cell"], ascending=True)
    result_table = ascending_order_table.reset_index(drop=True)
    print(result_table)
...
if __name__ == '__main__':
    language_dict = {'en': 'English', 'ar': 'Arabic', 'bn': 'Bengali', 'cs': 'Czech', 'da': 'Danish', 'de': 'German',
                     ...
                     'arn': 'Mapudungun', 'as': 'Assamese', 'az': 'Azeri', 'ba': 'Bashkir', 'be': 'Belarusian',
                     'bg': 'Bulgarian',
                     'bo': 'Tibetan', 'br': 'Breton', 'bs': 'Bosnian', 'ca': 'Catalan', 'co': 'Corsican', 'cy': 'Welsh',
                     'dsb': 'Lower Sorbian',
                     'dv': 'Divehi', 'et': 'Estonian', 'eu': 'Basque', 'fo': 'Faroese', 'fy': 'Frisian', 'ga': 'Irish',
                     'gd': 'Scottish Gaelic',
                     'gl': 'Galician', 'gsw': 'Alsatian', 'gu': 'Gujarati', 'ha': 'Hausa', 'hr': 'Croatian',
                     'hsb': 'Upper Sorbian', 'hy': 'Armenian',
                     'ig': 'Igbo', 'ii': 'Yi', 'is': 'Icelandic', 'iu': 'Inuktitut', 'ka': 'Georgian', 'kk': 'Kazakh',
                     'kl': 'Greenlandic', 'km': 'Khmer',
                     'kn': 'Kannada', 'kok': 'Konkani', 'ky': 'Kyrgyz', 'lb': 'Luxembourgish', 'lo': 'Lao',
                     'lt': 'Lithuanian', 'lv': 'Latvian', 'mi': 'Maori',
                     'mk': 'Macedonian', 'ml': 'Malayalam', 'mn': 'Mongolian', 'moh': 'Mohawk', 'mr': 'Marathi',
                     'ms': 'Malay', 'mt': 'Maltese', 'my': 'Burmese',
                     'nb': 'Norwegian (Bokmål)', 'ne': 'Nepali', 'nn': 'Norwegian (Nynorsk)', 'nso': 'Sesotho',
                     'oc': 'Occitan', 'or': 'Oriya', 'pa': 'Punjabi',
                     'prs': 'Dari', 'ps': 'Pashto', 'qut': 'K\'iche', 'quz': 'Quechua', 'rm': 'Romansh',
                     ...
                     'sms': 'Sami (Inari)', 'sq': 'Albanian', 'sr': 'Serbian', 'sw': 'Kiswahili', 'syr': 'Syriac',
